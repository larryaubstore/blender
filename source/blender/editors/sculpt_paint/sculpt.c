	...
	if (BKE_pbvh_node_get_tmin(node) < *tmin) {
		...
		if (srd->original && srd->ss->cache) {
			if (BKE_pbvh_type(srd->ss->pbvh) == PBVH_BMESH) {
				/* Note: the BMesh PBVH doesn't have an orco array, so
				 * here it is just set to 1 as a signal value */
				origco = (void*)0x1;
			}
			else {
		...
		if (BKE_pbvh_node_raycast(srd->ss->pbvh, node, origco, srd->ray_start, srd->ray_normal, &srd->dist)) {
			srd->hit = 1;
			*tmin = srd->dist;
		}
	}
