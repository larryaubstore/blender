...
static float vert_mask_get(BMesh *bm, BMVert *v)
{
	if (CustomData_has_layer(&bm->vdata, CD_PAINT_MASK)) {
		float *mask = CustomData_bmesh_get(&bm->vdata,
										   v->head.data,
										   CD_PAINT_MASK);
		return *mask;
	}
...
static GHash *bm_log_compress_ids_to_indices(unsigned int *ids, int totid)
{
	...
	for (i = 0; i < totid; i++) {
		BLI_ghash_insert(map,
						 SET_INT_IN_POINTER(ids[i]),
...
void BM_log_entry_drop(BMLogEntry *entry)
{
	...
	if (!entry->prev) {
		...
		assert(!"Cannot drop BMLogEntry from middle");
	}
...
const float *BM_log_original_vert_co(BMLog *log, BMVert *v)
{
	...
	assert(BLI_ghash_haskey(entry->modified_verts, key));
